...
11. **Create the Builder agent (once).** There is no public API (`/v1/agents` is not enabled). Preferred: the Bot opens [https://console.x.ai](https://console.x.ai) (Voice Agent Builder section) in its own browser with your approved console session, creates one agent, pastes [`prompts/voice-agent.md`](../prompts/voice-agent.md) **fully substituted once** (`{agent_name}` = `PHONEZERO_AGENT_NAME`; `{disclosure_clause}` = `, an automated assistant,` if `PHONEZERO_DISCLOSE_AI` is on, else empty), saves, and copies the `agentId`. Fallback: you do those same steps in the console and give the Bot the `agentId`. The prompt is static after paste. Do not add or edit a per-call TASK BRIEF in the Builder console — that step does not exist.
