...
#     only when PHONEZERO_ALLOWED_COUNTRIES is explicitly set
#     (the Telnyx voice profile is destination enforcement — not a
#     plugin variable. Leave an existing whitelist alone unless asked.)
...
Required environment:
  TELNYX_API_KEY
  PHONEZERO_FROM_NUMBER              E.164 DID already on the account (typically US).
                                     Also the SIP bridge target
                                     (sip:{FROM}@sip.voice.x.ai;transport=tls).

Optional:
  TELNYX_ACCOUNT_SID                 if unset, GET /v2/whoami → data.organization_id
  PHONEZERO_ALLOWED_COUNTRIES        developer-only. If set, writes that ISO
...
# Parse PHONEZERO_ALLOWED_COUNTRIES → JSON array of ISO 3166-1 alpha-2.
# Empty/unset → ["US"] (create default only). Telnyx profile is enforcement.
allowed_countries_json() {
  python3 -c '
import json, os, sys
raw = os.environ.get("PHONEZERO_ALLOWED_COUNTRIES") or "US"
codes = []
seen = set()
for part in raw.split(","):
    ...
    if len(c) != 2 or not c.isalpha():
        sys.stderr.write(
            "error: PHONEZERO_ALLOWED_COUNTRIES entries must be ISO 3166-1 alpha-2 (got %r)\n" % part
        )
        sys.exit(2)
    if c not in seen:
        seen.add(c)
        codes.append(c)
print(json.dumps(codes or ["US"]))
'
}
...
WHITELIST_JSON="$(allowed_countries_json)"
export WHITELIST_JSON
if [ "$WANT_WHITELIST_PATCH" -eq 1 ]; then
  echo "destination allowlist: ${WHITELIST_JSON} (PHONEZERO_ALLOWED_COUNTRIES → Telnyx profile)"
else
...
for item in (d.get("data") or []):
    ...
    if have_set != want_set:
        print("yes")
        print(",".join(sorted(have_set)) if have_set else "<empty>")
        print(",".join(sorted(want_set)))
    else:
        print("no")
    raise SystemExit(0)
print("yes")
...
' <"${WORKDIR}/profiles.json"
    ...
    echo "outbound voice profile whitelist unchanged (Telnyx has [${HAVE_WL}]; set PHONEZERO_ALLOWED_COUNTRIES to write)"
  elif [ "$DRIFT_FLAG" = "yes" ]; then
    HAVE_WL="$(printf '%s\n' "$DRIFT" | sed -n '2p')"
    WANT_WL="$(printf '%s\n' "$DRIFT" | sed -n '3p')"
    python3 -c '
import json, os
print(json.dumps({"whitelisted_destinations": json.loads(os.environ["WHITELIST_JSON"])}))
' >"${WORKDIR}/profile-patch.body"
    PROFILE_ENC="$(urlencode "$PROFILE_ID")"
    if [ "$DRY_RUN" -eq 1 ]; then
      echo "whitelist drift: profile has [${HAVE_WL}], configured [${WANT_WL}] — would PATCH /v2/outbound_voice_profiles/${PROFILE_ID}"
      python3 -c 'import sys; print(sys.stdin.read())' <"${WORKDIR}/profile-patch.body"
    else
      CODE="$(telnyx_http PATCH "https://api.telnyx.com/v2/outbound_voice_profiles/${PROFILE_ENC}" "${WORKDIR}/profile-patch.json" "${WORKDIR}/profile-patch.body")" || true
      if [ "$CODE" != "200" ]; then
        echo "error: PATCH /v2/outbound_voice_profiles/{id} HTTP ${CODE}" >&2
        python3 -c 'import sys; print(sys.stdin.read()[:400])' <"${WORKDIR}/profile-patch.json" >&2
        exit 1
      fi
      echo "updated outbound voice profile whitelist to ${WHITELIST_JSON}"
    fi
  else
...
print(json.dumps({
    ...
    "whitelisted_destinations": json.loads(os.environ["WHITELIST_JSON"]),
...
if [ "$INBOUND_CODE" != "200" ]; then
  INBOUND_MSG="PHONEZERO_INBOUND_XML_URL HTTP ${INBOUND_CODE} (expected 200). Set PHONEZERO_INBOUND_XML_URL to a reachable raw URL for texml/inbound.xml."
  if [ "$DRY_RUN" -eq 1 ]; then
...
' <"${WORKDIR}/xai-numbers.json"
  ...
  if [ -n "$XAI_INFO" ]; then
    ...
    echo "found xAI phone-number ${PHONEZERO_FROM_NUMBER} (phoneNumberId=${XAI_PN_ID} origin=${XAI_ORIGIN:-unset})"
...
print(json.dumps({
    "name": "PhoneZero",
    "phoneNumber": os.environ["PHONEZERO_FROM_NUMBER"],
    "origin": "byo_trunk",
}))
...
' <"${WORKDIR}/xai-reg.json"
      ...
      echo "registered ${PHONEZERO_FROM_NUMBER} with xAI as byo_trunk (${XAI_PN_ID})"
    fi
  fi
...
echo "SIP bridge target: sip:${PHONEZERO_FROM_NUMBER}@sip.voice.x.ai;transport=tls"
echo
echo "Other ids"
echo "---------"
