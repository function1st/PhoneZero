...
- `prompts/voice-agent.md` v1: opening line self-identifies as an AI assistant **and gives recording notice** ("this call may be recorded for accuracy"), states the ask early, negotiates only within the window, verbatim read-back before accepting, then **closes every call with the spoken structured recap** ("Confirming: booked / not booked, {time}, party of {n}, under {name}"). If the callee objects to recording or AI, end politely and report `needs_user`. Voicemail policy (leave callback, don't book).
...
| Recording/transcription consent laws | **Notice, not storage tricks, is the mitigation**: deleting audio after transcription does not change the legal act of capture, and real-time transcription is interception in the same all-party-consent states (see the CIPA suits against AI call-transcription services). The agent's opening line therefore always includes "this call may be recorded for accuracy" — the standard consent-by-continuing mechanism — and the agent is itself a party (covers federal + one-party states). If the callee objects, the agent ends the call and reports `needs_user`. Defaults: audio deleted immediately after transcription, transcript kept only until the outcome is confirmed, calling limited to the user's home country (US-first; international out of scope v1 — some jurisdictions, e.g. Germany, criminalize recording regardless of casual notice). README carries the compliance section |
