...
#   XAI_API_KEY — if set, verify BYO registration of PHONEZERO_FROM_NUMBER
set -euo pipefail
...
# From is always the US DID. Dest country enforcement is the Telnyx
# voice profile, not this script.
is_e164() {
  [[ "$1" =~ ^\+[1-9][0-9]{6,14}$ ]]
}
...
check_number() {
  ...
  local num="$2"
  if ! is_e164 "$num"; then
    fail "${name} is not E.164 (e.g. +15555550100)"
    return 1
  fi
  if ! is_us_e164 "$num"; then
    fail "${name} is not a US E.164 DID (+1 and 10 digits)"
    return 1
  fi
  pass "${name} is US E.164"
  return 0
}
...
# From is the US DID. Dest countries are the Telnyx voice profile.
check_number "PHONEZERO_FROM_NUMBER" "$PHONEZERO_FROM_NUMBER" || true
...
' <"${WORKDIR}/xai-numbers.json"
    ...
    if [ "$XAI_MATCH" != "yes" ]; then
      fail "PHONEZERO_FROM_NUMBER not registered with xAI (GET /v2/phone-numbers)"
    else
